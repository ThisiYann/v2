...
function ArtHub:Window(GuiConfig)
    ...
    local ArtScreen = Instance.new("ScreenGui");
    local DropShadowHolder = Instance.new("Frame");
    local DropShadow = Instance.new("ImageLabel");
    ...
    ArtScreen.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    ArtScreen.Name = "ArtScreen"
    ArtScreen.ResetOnSpawn = false
    ArtScreen.Parent = game:GetService("CoreGui")
    ...
    DropShadowHolder.ZIndex = 0
    DropShadowHolder.Name = "DropShadowHolder"
    DropShadowHolder.Parent = ArtScreen

    DropShadowHolder.Position = UDim2.new(0, (ArtScreen.AbsoluteSize.X // 2 - DropShadowHolder.Size.X.Offset // 2), 0,
        (ArtScreen.AbsoluteSize.Y // 2 - DropShadowHolder.Size.Y.Offset // 2))
    ...
    function GuiFunc:DestroyGui()
        if CoreGui:FindFirstChild("ArtScreen") then
            ArtScreen:Destroy()
        end
    end
    ...
    Close.Activated:Connect(function()
        ...
        Dialog.Image = "rbxassetid://100060752356468"
        ...
        Gradient.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0.0, Color3.fromRGB(36, 36, 36)),
            ColorSequenceKeypoint.new(0.25, Color3.fromRGB(255, 255, 255)),
            ColorSequenceKeypoint.new(0.5, Color3.fromRGB(36, 36, 36)),
            ColorSequenceKeypoint.new(0.75, Color3.fromRGB(255, 255, 255)),
            ColorSequenceKeypoint.new(1.0, Color3.fromRGB(36, 36, 36))
        })
        ...
        Yes.MouseButton1Click:Connect(function()
            if ArtScreen then ArtScreen:Destroy() end
    ...
    UIStroke14.Color = Color3.fromRGB(0, 0, 0)
    UIStroke14.Thickness = 2.5
    UIStroke14.Transparency = 0.8
    UIStroke14.Parent = DropdownSelect

    DropdownSelectReal.AnchorPoint = Vector2.new(0.5, 0.5)
    DropdownSelectReal.BackgroundColor3 = Color3.fromRGB(36, 36, 36)
...
for i, connection in ipairs(getconnections(SERVICES.LP.Idled)) do
    connection:Disable()
end

SERVICES.LP.Idled:Connect(function()
    if VirtualUser then
        VirtualUser:Button(Vector2.new(0, 0), workspace.CurrentCamera.CFrame)
        task.wait(1)
        VirtualUser:Button(Vector2.new(0, 0), workspace.CurrentCamera.CFrame)
    end
end)
...
local MODULES = {
    ...
    Replion = SERVICES.RS.Packages.Replion,
    FishingController = SERVICES.RS.Controllers.FishingController,
    TradingController = SERVICES.RS.Controllers.ItemTradingController,
    ItemUtility = SERVICES.RS.Shared.ItemUtility,
    VendorUtility = SERVICES.RS.Shared.VendorUtility,
    PlayerStatsUtility = SERVICES.RS.Shared.PlayerStatsUtility,
    Effects = SERVICES.RS.Shared.Effects,
    NotifierFish = SERVICES.RS.Controllers.TextNotificationController
};
...
local tpList = {
    ["Clasic"] = Vector3.new(1218.8, 7.0, 2842.6),
    ["Iron Cavern"] = Vector3.new(-8794.4, -585.0, 225.5),
    ["Iron Cave"] = Vector3.new(-8642.2, -547.5, 160.5),
    ...
    ["Creater Island"] = Vector3.new(998.0, 2.9, 5151.2),
    ...
    ["Sacted Temple"] = Vector3.new(1451.4, -22.1, -635.7),
...
local listEventTele = EventList
local currentSelectedLocation = nil;
local currentSelectedPlayer = nil;
local autoEventTeleportState = false;
local autoEventTargetName = EventList[1];
local autoEventTeleportThread = nil;

-- FUNCTION
local function TeleportTo(CFrameOrVector)
    local char = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait();
    local HRP = char:FindFirstChild("HumanoidRootPart");
    if HRP then
        if typeof(CFrameOrVector) == "Vector3" then
            HRP.CFrame = CFrame.new(CFrameOrVector);
        elseif typeof(CFrameOrVector) == "CFrame" then
            HRP.CFrame = CFrame.new(CFrameOrVector.p);
        end
    end
end
...
local listPlayersTele = GetPlayerList();
...
local function EquipRod()
    local function IsRodEquipped()
        local equippedId = loadData.Data:Get("EquippedId")
        if not equippedId then
            return false
        end
        local item = MODULES.PlayerStatsUtility:GetItemFromInventory(loadData.Data, function(data)
            return data.UUID == equippedId
        end)
        if not item then
            return false
        end
    ...
    if not IsRodEquipped() then
        Remote.RE.EquipTool:FireServer(1)
    end
end
...
local function InstantFishing()
    ...
    task.spawn(function()
        while _G.InstantStatus do
            local v195, _, v197 = pcall(function()
                return Remote.RF.ChargeRod:InvokeServer(workspace:GetServerTimeNow());
            end);
            do
                local l_v197_0 = v197;
                if v195 and typeof(l_v197_0) == "number" then
                    ...
                    do
                        ...
                        pcall(function()
                            Remote.RF.StartMini:InvokeServer(l_v199_0, l_v200_0, l_v197_0);
                        end);
                        local v203 = tick();
                        repeat
                            task.wait(0.05);
...
local function Fastest()
    task.spawn(function()
        pcall(function()
            Remote.RF.Cancel:InvokeServer()
        end)
        local ServerTime = workspace:GetServerTimeNow()
        pcall(function()
            Remote.RF.ChargeRod:InvokeServer(ServerTime)
        end)
        pcall(function()
            Remote.RF.StartMini:InvokeServer(-1, 0.999)
        end)
        task.wait(_G.BlatantReel)
        pcall(function()
            Remote.RE.FishDone:FireServer()
        end)
    end)
...
local function FindAndTeleportToTargetEvent()
    ...
    if success and targetCFrame and typeof(targetCFrame) == "CFrame" then
        ...
        arthub("Event Found: Teleported to " .. targetName);
        return true
    end
...
local function RunAutoEventTeleportLoop()
    ...
    autoEventTeleportThread = task.spawn(function()
        arthub("Auto Event TP ON: Scanning for " .. autoEventTargetName);
